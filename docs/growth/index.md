# 独自升级

这里放主动学习、能力拓展、应用题推演和技术选型。它们不一定来自一次线上事故，但能补齐后端工程师的系统化思考能力。

## AI 与智能体

- [Spring AI 智能体学习复盘](/growth/spring-ai-music-agent)
- [Dify 工作流：钢银提货助手流程地图](/growth/dify-delivery-assistant-flow-map)

## 系统设计与性能推演

- [Spring Boot / Spring Cloud 组件模块梳理](/growth/spring-components)
- [MQ&分布式锁技术选型横向对比](/growth/mq-distributed-lock-technology-comparison)
- [超时&重复&慢怎么办](/growth/timeout-duplicate-slow-questions)
- [系统性能瓶颈与排查](/growth/system-performance-bottlenecks)
- [高可用强一致性的设计思路](/growth/high-availability-strong-consistency-design)
- [Hikari 连接池与数据库连接数](/growth/hikari-connection-pool)
- [DDD 入门：从三层架构到业务建模](/growth/domain-driven-design)

## 安全与认证

- [加密基础：对称加密、非对称加密](/growth/encryption-basics)
- [登录认证Cookie、Session、JWT](/growth/authentication-cookie-session-jwt)
